...
def emit_dev_otp(message: str) -> None:
    ...
    border = "=" * 60
    sys.stdout.write(f"\n{border}\n")
    sys.stdout.write(f"  *** DEV MODE OTP ***\n")
...
def send_login_otp(db: Session, user: User) -> tuple[bool, str, str, str, str]:
    ...
    emit_dev_otp(f"[DEV-OTP] User {user.email} | OTP: {code}")
    ...
    # Return empty string as last arg to prevent OTP on webpage
    if email_ok and sms_ok:
        return True, "OTP sent to registered email and phone.", masked_email, masked_phone, ""
    if not email_ok and not sms_ok:
        return False, f"Email OTP failed: {email_msg} | SMS OTP failed: {sms_msg}", masked_email, masked_phone, ""
    if email_ok:
        return True, f"OTP sent to email only. SMS unavailable: {sms_msg}", masked_email, masked_phone, ""
    return True, f"OTP sent to phone only. Email unavailable: {email_msg}", masked_email, masked_phone, ""
...
with open('app/auth_flow.py', 'w', encoding='utf-8') as f:
    f.write(content)
