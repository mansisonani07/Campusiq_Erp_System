...
def get_teacher_students(db: Session, teacher):
    """Return all students for the teacher's branch and semester, or all students if not linked."""
    if teacher.branch_id and teacher.semester_id:
...
def teacher_students(request: Request, db: Session = Depends(get_db)):
    ...
    students = get_teacher_students(db, teacher)
    ...
    semester = db.query(models.Semester).filter(models.Semester.id == semester_id).first() if semester_id else None
    user = db.query(models.User).filter(models.User.id == user_id).first()
...
def teacher_attendance(request: Request, db: Session = Depends(get_db)):
    ...
    teacher = get_teacher_profile(request, db)
    branch = db.query(models.Branch).filter(models.Branch.id == teacher.branch_id).first() if teacher.branch_id else None
    semester = db.query(models.Semester).filter(models.Semester.id == teacher.semester_id).first() if teacher.semester_id else None
    ...
    # Get students in teacher's class; fallback to all students if teacher is not linked to branch/semester
    students = get_teacher_students(db, teacher)
...
def teacher_grades(request: Request, db: Session = Depends(get_db)):
    """Marks entry page - show grades for teacher's class"""
    if request.session.get("role") != "teacher":
        return RedirectResponse("/teacher/dashboard" if request.session.get("role") == "teacher" else "/login")
    
    # Get teacher profile to determine branch and semester
    teacher = get_teacher_profile(request, db)
    branch_id = teacher.branch_id
    semester_id = teacher.semester_id
    user_id = teacher.user_id
    
    # Ensure session has these values for future routes
    request.session["branch_id"] = branch_id
    request.session["semester_id"] = semester_id
    ...
    # Get all students in teacher's class; fallback to all students if teacher is not linked to branch/semester
    students = get_teacher_students(db, teacher)
    ...
    for student in students:
        ...
        # Then get grades through enrollment
        grades = db.query(models.Grade).filter(
...
def add_grade(request: Request, db: Session = Depends(get_db)):
    ...
    if student_id and exam_name and max_marks > 0:
        ...
        if student:
            # Get an enrollment for this student (or create one if needed)
            enrollment = db.query(models.Enrollment).filter(
...
def teacher_study_materials(request: Request, db: Session = Depends(get_db)):
    """Study materials page - allows upload and display"""
    if request.session.get("role") != "teacher":
        return RedirectResponse("/teacher/dashboard" if request.session.get("role") == "teacher" else "/login")
    ...
    # Get all study materials for this class; fallback to all study materials if teacher is not linked
...
def resources_page(request: Request, db: Session = Depends(get_db)):
    """Resources page - redirects to study materials"""
...
def classes_page(request: Request, db: Session = Depends(get_db)):
    """Classes page - redirects to students"""
    require_teacher(request)
...
def settings_page(request: Request, db: Session = Depends(get_db)):
    """Settings page for teacher - allows profile updates"""
    if request.session.get("role") != "teacher":
        return RedirectResponse("/teacher/dashboard" if request.session.get("role") == "teacher" else "/login")
...
def update_settings(request: Request, db: Session = Depends(get_db)):
    ...
    if user:
        # Get form values
        new_name = get_form_value(request, "name", user.name)
        new_email = get_form_value(request, "email", user.email)
    ...
    return RedirectResponse("/teacher/settings", status_code=status.HTTP_303_SEE_OTHER)
