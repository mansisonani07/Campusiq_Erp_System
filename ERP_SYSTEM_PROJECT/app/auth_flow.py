...
def emit_dev_otp(message: str) -> None:
    """Force OTP visibility during local development - simplified approach."""
    # Print to terminal
    print(f"\n{'='*60}", flush=True)
    ...
    # Also write to a text file for easy access
    try:
        with open("otp_display.txt", "w", encoding="utf-8") as f:
            f.write(f"{'='*60}\n")
            f.write(f"*** OTP FOR LOGIN ***\n")
            f.write(f"{message}\n")
            f.write(f"{'='*60}\n")
    except Exception:
        pass
...
def send_login_otp(db: Session, user: User) -> tuple[bool, str, str, str, str]:
    ...
    # Always show OTPs for development - force visibility regardless of environment
    message = f"[DEV-OTP] User {user.email} | OTP: {code}"
    ...
    if not sms_target:
        ...
        if not phone and sms_ok:
            sms_msg = f"{sms_msg} (using terminal fallback target)"
    ...
    if email_ok and sms_ok:
        return True, "OTP sent to registered email and phone.", masked_email, masked_phone, dev_otp
    if not email_ok and not sms_ok:
        return False, f"Email OTP failed: {email_msg} | SMS OTP failed: {sms_msg}", masked_email, masked_phone, dev_otp
    if email_ok:
        return True, f"OTP sent to email only. SMS unavailable: {sms_msg}", masked_email, masked_phone, dev_otp
    return True, f"OTP sent to phone only. Email unavailable: {email_msg}", masked_email, masked_phone, dev_otp
...
def ensure_device_approved_or_send_request(db: Session, request: Request, user: User) -> tuple[bool, str]:
    ...
    approve_url = f"{base_url}/approve-device?token={raw_token}"
    subject = "New Device Login Attempt - Approval Required"
    body = (
        f"We detected a login attempt from a new device.\n\n"
        f"Device: {device_name}\nIP: {get_client_ip(request)}\n"
        f"Approve this device: {approve_url}\n\n"
